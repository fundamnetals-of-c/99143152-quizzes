...
if(ip.octet2 < 0 || ip.octet2 > 255)
{
return 0;
}
if(ip.octet3 < 0 || ip.octet3 > 255)
{
return 0;
}
if(ip.octet4 < 0 || ip.octet4 > 255)
{
return 0;
}
return 1;
	...
	for(i=0; i<addr_array_len; i++)
{
	...
	for(i=0; i<addr_array_len + 1; i++){
if(addr_array_len != 4){printf(">\n");}
		char input[50];
...
{
		if(addr_array_len != 4){printf("Invalid input\n");}
}
}	
...
void print_same_subnet(const ipaddr_t addr_array[], int addr_array_len) 
{
/*ipaddr_t temp[addr_array_len];
ipaddr_t first = addr_array[0];*/
int arr[addr_array_len];
ipaddr_t ipleft;
int i=0;
	...
	for(i=0; i<addr_array_len && j<addr_array_len; i++){
